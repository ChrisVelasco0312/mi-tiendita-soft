...
from textual.widgets import DataTable, Header, Label, Select
...
class ManageSellView(Screen):
        ...
        yield Grid(
            ...
            Horizontal(
                ...
                Container(
                    ...
                    Label(
                        "Selecciona una venta para ver la fecha",
                        id="sell_date_label",
                        classes="date-label",
                    ),
    ...
    def load_sell_detail(self, sell_id):
        ...
        quantities = (
            selected_sell["quantities"].split("-")
            if selected_sell["quantities"]
            else []
        )
        ...
        for i, item_code in enumerate(items):
            if i < len(quantities):
                ...
                # Buscar la información del producto en el stock
                ...
                if product_info.empty:
                    try:
                        ...
                        product_info = stock_data[
                            stock_data["item_code"] == item_code_int
                        ]
    ...
    def get_date_range(self, filter_value):
        ...
        if filter_value == "all":
            ...
            start_date = today - timedelta(days=6)  # Incluido hoy = 7 días
            return start_date, today
        elif filter_value == "last_30_days":
            start_date = today - timedelta(days=29)  # Incluido hoy = 30 días
            return start_date, today
        elif filter_value == "this_month":
            start_date = today.replace(day=1)
            return start_date, today
        elif filter_value == "last_month":
            # Se toma el primer día del mes actual
            first_day_current_month = today.replace(day=1)
            # Luego se resta 1 día para obtener el último día del més previo
    ...
    def filter_sell_data_by_date(self, filter_value):
        ...
        filtered_data = sell_data[
            (sell_data["date"] >= start_date) & (sell_data["date"] <= end_date)
        ]
